    ...
    <>
      ...
      >
        <Image
          ...
          height={32}
        />
        <span className="absolute left-10 bottom-1 bg-foreground text-background text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none">
          Consent Preference
        </span>
      </motion.div>
      ...
      <AnimatePresence>
        {isOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center">
            ...
            {/* Modal */}
            <motion.div
              className="relative bg-background rounded-2xl w-[50%] mx-auto max-h-[85vh] flex flex-col shadow-xl"
              initial={{ opacity: 0, y: 100, scale: 0.95 }}
              animate={{ opacity: 1, y: 0, scale: 1 }}
              exit={{ opacity: 0, y: 100, scale: 0.95 }}
              transition={{ type: 'spring', damping: 25, stiffness: 300 }}
            >
              <div className="sticky top-0 z-10 bg-background flex items-center justify-between p-5 border-b rounded-t-2xl overflow-hidden">
                <h2
                  className="text-lg font-bold px-4 text-foreground"
                >
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-3">
                ...
                {CATEGORIES.map((cat, index) => (
                  ...
                  >
                    <button
                      type="button"
                      className="w-full flex items-center gap-2 text-left p-3 font-medium border rounded-lg hover:bg-surface-subtle transition-colors text-foreground"
                    ...
                    >
                      <div className="flex flex-col gap-1 w-full">
                        ...
                        <AnimatePresence>
                          {expanded === cat.name && (
                            <motion.div
                              className="text-xs text-muted-foreground bg-neutral rounded px-3 py-2 mt-1 w-full ml-5"
                              initial={{ opacity: 0, height: 0 }}
                              animate={{ opacity: 1, height: 'auto' }}
                              exit={{ opacity: 0, height: 0 }}
                              transition={{ duration: 0.2 }}
              ...
              <div className="flex items-center gap-3 p-5 border-t">
                <motion.button
                  className="flex-1 px-4 py-2 text-sm font-medium text-foreground border border-border py-3 h-fit rounded-sm hover:bg-surface-subtle transition-colors"
                ...
                <motion.button
                  className="flex-1 px-4 py-2 text-sm font-medium text-primary-foreground bg-primary py-3 h-fit rounded-sm hover:opacity-90 transition-opacity"
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                  onClick={() => setIsOpen(false)}
                >
