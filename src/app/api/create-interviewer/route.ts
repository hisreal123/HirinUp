...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (existingLisas.length > 1) {
      logger.info(`Found ${existingLisas.length} Lisa interviewer(s), keeping first one (id: ${existingLisa?.id}), deleting ${existingLisas.length - 1} duplicate(s)...`);
      for (let i = 1; i < existingLisas.length; i++) {
        try {
          await InterviewerService.deleteInterviewer(existingLisas[i].id);
          logger.info(`Deleted duplicate Lisa interviewer with id: ${existingLisas[i].id}`);
        } catch (error: any) {
          logger.warn(`Could not delete duplicate Lisa interviewer ${existingLisas[i].id}:`, error?.message);
        }
      }
    }

    if (existingBobs.length > 1) {
      logger.info(`Found ${existingBobs.length} Bob interviewer(s), keeping first one (id: ${existingBob?.id}), deleting ${existingBobs.length - 1} duplicate(s)...`);
      for (let i = 1; i < existingBobs.length; i++) {
        try {
          await InterviewerService.deleteInterviewer(existingBobs[i].id);
          logger.info(`Deleted duplicate Bob interviewer with id: ${existingBobs[i].id}`);
        } catch (error: any) {
          logger.warn(`Could not delete duplicate Bob interviewer ${existingBobs[i].id}:`, error?.message);
        }
      }
    }
    ...
    // Create or update Lisa
    {
      try {
        ...
        if (existingLisa) {
          ...
          logger.info("Updating existing Lisa interviewer with new agent_id (preserving ID for existing interviews)...");
        ...
        logger.error("Error creating/updating Lisa interviewer:", error?.message || error);
    ...
    // Create or update Bob
    {
      try {
        ...
        if (existingBob) {
          // Update existing interviewer to preserve foreign key references in interviews
          logger.info("Updating existing Bob interviewer with new agent_id (preserving ID for existing interviews)...");
          newSecondInterviewer = await InterviewerService.updateInterviewerAgentId(
            existingBob.id,
        ...
        logger.error("Error creating/updating Bob interviewer:", error?.message || error);
        console.error("Full error:", error);
      }
    }
    ...
    if (newInterviewer?.agent_id) {
      try {
        lisaAgentVerified = await retellClient.agent.retrieve(newInterviewer.agent_id);
    ...
    if (newSecondInterviewer?.agent_id) {
      try {
        bobAgentVerified = await retellClient.agent.retrieve(newSecondInterviewer.agent_id);
