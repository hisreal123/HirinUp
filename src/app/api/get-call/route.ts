...
return NextResponse.json(
    ...
    const callDetails: Response | null =
      await ResponseService.getResponseByCallId(body.id);
    ...
    try {
      ...
      if (!saveResult || saveResult.length === 0) {
        logger.error('Failed to save response - no rows updated:', {
          ...
          saveResult,
          hasCallResponse: !!callResponse,
          callResponseKeys: callResponse
            ? Object.keys(callResponse).slice(0, 10)
            : [],
        });

        // Try to check if response exists
        const checkResponse = await ResponseService.getResponseByCallId(
          body.id
        );
        ...
        logger.info('Successfully saved response details:', {
          ...
          detailsKeys: saveResult[0]?.details
            ? Object.keys(saveResult[0].details).slice(0, 10)
            : [],
