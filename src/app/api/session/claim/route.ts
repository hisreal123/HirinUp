...
export async function POST(req: Request) {
  try {
    ...
    const { data: response, error: fetchError } = await supabase
      .from("response")
      .select("id, active_session_id, session_fingerprint, last_heartbeat, is_ended")
      .eq("token", token)
      .single();

    if (fetchError || !response) {
      logger.error("[Session Claim] Response not found:", { token, error: fetchError });
      return NextResponse.json(
    ...
    if (response.is_ended) {
      return NextResponse.json(
        { error: "Interview has already ended", message: "This interview link has expired" },
        { status: 410 }
      );
    }
    ...
    logger.info("[Session Claim] Session claimed successfully:", { token, session_id });
