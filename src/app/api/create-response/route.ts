...
export async function POST(req: Request) {
  try {
    ...
    if (isCandidateSubmission) {
      if (!turnstile_token) {
        logger.warn('Candidate submission without turnstile_token', {
          interview_id,
          call_id,
        });
      ...
      logger.info('create-response request received (candidate verified)', {
        interview_id,
        email,
      });
    } else {
      // Admin link generation - no Turnstile required
      logger.info('create-response request received (admin link generation)', {
        interview_id,
      });
    }
      ...
      return NextResponse.json(
        {
          ...
          details: 'No response ID returned',
        },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json({ response_id: responseToken }, { status: 200 });
  } catch (err: any) {
