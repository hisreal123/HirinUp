...
const SECTION_CLASS = "mb-4 text-xl font-semibold leading-[53px] tracking-wider text-gray-500";
      ...
      <div className="mx-auto max-w-7xl px-6 pt-10 pb-10">
        <div className="grid grid-cols-1 gap-16 md:grid-cols-3 justify-center text-center">
          ...
          <div>
            ...
            <LinkList links={legalLinks} className="mb-6 space-y-3 text-sm text-white" />
            ...
            {getContactItems().map(({ href, label }) => (
              <p key={href} className="text-sm text-gray-400">
                <a href={href} className="hover:text-purple-600">{label}</a>
              </p>
            ))}
          </div>
        </div>
        ...
        <div className="mt-16 flex flex-col items-center justify-between gap-6 border-t border-white/10 pt-6 md:flex-row">
          <p className="text-xs text-gray-500">© 2025 Talvin. All Rights Reserved.</p>
          <div className="flex items-center gap-4 text-gray-400">
            {socialLinks.map(({ href, label, path, viewBox }) => (
              <Link
                ...
                aria-label={label}
              >
                <svg aria-hidden className="w-6 h-6" fill="#ffffff" viewBox={viewBox} xmlns="http://www.w3.org/2000/svg">
                  <path d={path} />
                </svg>
              </Link>
            ))}
